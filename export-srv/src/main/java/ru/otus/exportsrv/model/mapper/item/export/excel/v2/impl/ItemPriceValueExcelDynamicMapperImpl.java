...
public class ItemPriceValueExcelDynamicMapperImpl implements ItemPriceValueExcelDynamicMapper {

    private final Map<String, Field> declaredFieldMap = Arrays.stream(ItemPriceExport.class.getDeclaredFields())
            .collect(toMap(Field::getName, Function.identity()));

    @Override
    public void map(Workbook workbook, List<ItemPriceExport> prices, List<ExportSheetSettings> sheetSettings) {

        if (isNull(workbook) || isEmpty(prices)) {
            return;
        }
        var sheetSetting = sheetSettings.stream()
                .filter(f -> ExportObject.ITEM_PRICE.getName().equals(f.getObjectName()))
        ...
        CellStyle gefaultCellStyle = getDefaultCellStyle(workbook);
        ...
        for (ExportObjectSettings settings : sheetSetting.getObjectSettings()) {
            ...
            if (nonNull(field)) {
                ...
                cell.setCellStyle(gefaultCellStyle);
        ...
        for (var price : prices) {
            ...
            for (var settings : sheetSetting.getObjectSettings()) {

                int columnIndex = settings.getColumnIndex() - 1;
                Field field = declaredFieldMap.get(settings.getFiledName());

                if (nonNull(field)) {
                    Object value = getFieldValue(price, field);
